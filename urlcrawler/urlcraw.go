...
func downloadImg(imgId string) bool {
	...
	data := detail.Data
	urls := data.ImageUrls
	if len(urls) < 1 || data.TotalBookmarks < 500 || data.Type != tag{
		return false
	}
	// 拼接图片地址URL
	pictureUrl := &url.URL{}
	...
	// 根据图片的尺寸信息确定图片归属
	bathPath := "images/"
	if data.Width >= 1800 && data.Width > data.Height && data.Height >=1000 {
		bathPath += "宽图/"
	} else if data.Height >= 1800 && data.Height >= data.Width && data.Width >= 1000 {
		bathPath += "长图/"
	} else {
		bathPath += "小图/"
	}
...
func (p *Pixivic) GetRelevanceUrls(imgId string) {
	...
	for i := 1; i <= 10; i++ {
		originUrl := "https://api.pixivic.com/illusts/" +
			imgId + "/related?page=" + strconv.Itoa(i) + "&pageSize=100"
